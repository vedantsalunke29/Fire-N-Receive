...
import { ArrowUp, ArrowDown, SortAsc, Loader2, Maximize2, Minimize2 } from 'lucide-react'
...
export const ResultsTable = ({ data, columns, isLoading }: ResultsTableProps) => {
  ...
  const loadMoreData = useCallback(() => {
    ...
    const nextBatch = data.slice(loadedDataSize, loadedDataSize + nextBatchSize)
  ...
  const calculateVisibleRows = useCallback(() => {
    ...
    if (headerRef.current && scrollRef.current) {
      if (headerRef.current?.parentElement) {
        headerRef.current.parentElement.scrollLeft = scrollRef.current.scrollLeft
      }
    }
  ...
  const getSortIcon = (column: string) => {
    if (sortColumn !== column) {
      return <SortAsc size={14} className="sort-icon" />
    }
  ...
  return (
    <div className={`results-section ${isFullscreen ? 'fullscreen' : ''}`} ref={containerRef}>
      <div className="results-header">
        <h2>Results</h2>
        <div className="results-meta">
          {data.length > 0 && <span className="results-count">{data.length} rows returned</span>}
          {loadedDataSize < data.length && <span className="loaded-count">({loadedDataSize} loaded)</span>}
          <button 
      ...
      <div className="table-container">
        ...
        <div className="table-scroll" ref={scrollRef}>
          <div className="virtual-scroll-container" style={{ height: `${totalHeight}px` }}>
            <table
          ...
          <div className="scroll-info">
            {isScrolling && (
              <div className="scrolling-indicator">
                ...
                <span>
                  Viewing rows {startIndex + 1}-{Math.min(endIndex, loadedDataSize)} of {data.length}
                </span>
              </div>
            )}
