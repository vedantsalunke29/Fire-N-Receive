...
export const QueryEditor: React.FC<QueryEditorProps> = ({ value, onChange }) => {
  ...
  const updateHeight = useCallback(() => {
    if (textareaRef.current) {
      ...
      const maxVisibleRows = isFullscreen ? MAX_ROWS_FULLSCREEN : MAX_ROWS_NORMAL

      const newHeight = Math.max(Math.min(maxVisibleRows, lineCount) * LINE_HEIGHT, 200)
  ...
  const syncScroll = (e: React.UIEvent<HTMLTextAreaElement>) => {
    const lineNumbersElement = e.currentTarget.parentElement?.previousSibling as HTMLElement
  ...
  return (
    <div ref={editorRef} className={`query-editor-container ${isFullscreen ? "fullscreen" : ""}`}>
      ...
      <div className="editor-content" style={{ height: isFullscreen ? 'calc(100vh - 40px)' : `${editorHeight}px`  }}>
